...
public class cvc_complex_type_2_4_bCodeAction implements ICodeActionParticipant {
	...
	public void doCodeAction(ICodeActionRequest request, List<CodeAction> codeActions, CancelChecker cancelChecker) {
		...
		try {
			...
			int elementOffset = element.getEndTagOpenOffset();
			...
			Collection<String> existingElements = new TreeSet<String>(Collator.getInstance());
			for (DOMNode root : document.getRoots()) {
				existingElements.add(root.getNodeName());
				Collection<DOMNode> existingChilren = root.getChildren();
				for (DOMNode existingChild : existingChilren) {
					existingElements.add(existingChild.getNodeName());
				}
			}
